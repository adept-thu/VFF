...
class Tj4dDatasetRadar(DatasetTemplate):
    ...
    def include_kitti_data(self, mode):
        if self.logger is not None:
            self.logger.info('Loading tj4d dataset')
        kitti_infos = []
        ...
        if self.logger is not None:
            self.logger.info('Total samples for Tj4d dataset: %d' % (len(kitti_infos)))
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        db_info_save_path = Path(self.root_path) / ('tj4d_dbinfos_%s.pkl' % split)
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        eval_det_annos = copy.deepcopy(det_annos) # 检测结果
    ...
    def __getitem__(self, index):
        ...
        info = copy.deepcopy(self.kitti_infos[index]) # kitti_infos是数据集样本信息，字典的列表
...
def create_tj4d_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = Tj4dDatasetRadar(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
    train_split, val_split = 'train', 'val'

    train_filename = save_path / ('tj4d_infos_%s.pkl' % train_split)
    val_filename = save_path / ('tj4d_infos_%s.pkl' % val_split)
    trainval_filename = save_path / 'tj4d_infos_trainval.pkl'
    test_filename = save_path / 'tj4d_infos_test.pkl'
    ...
    print('tj4d info train file is saved to %s' % train_filename)

    dataset.set_split(val_split)
    kitti_infos_val = dataset.get_infos(num_workers=workers, has_label=True, count_inside_pts=True)
    with open(val_filename, 'wb') as f:
        pickle.dump(kitti_infos_val, f)
    print('tj4d info val file is saved to %s' % val_filename)

    with open(trainval_filename, 'wb') as f:
        pickle.dump(kitti_infos_train + kitti_infos_val, f)
    print('tj4d info trainval file is saved to %s' % trainval_filename)

    dataset.set_split('test')
    kitti_infos_test = dataset.get_infos(num_workers=workers, has_label=False, count_inside_pts=False)
    with open(test_filename, 'wb') as f:
        pickle.dump(kitti_infos_test, f)
    print('tj4d info test file is saved to %s' % test_filename)
...
if __name__ == '__main__':
    import sys
    import pdb
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_tj4d_infos':
        import yaml

    ...
    def forward(self, voxel_coords, batch_dict, layer_name):
        ...
        Returns:
            ...
            对离散的点
                ...
                point_mask: (B, N), Useful points indictor
            对连续的体素
                ray_grid: (B, N, 2), 体素投影得到的Image coordinates in X,Y of image plane
                ray_depths: (B, N), 体素投影得到的Image depth
                voxel_grid：(B, N, 3)体素场上点的坐标
                lidar_grid：未进行数据增强的原始lidar的坐标
        ...
        if 'ray' in self.fuse_mode:
            """
            第二路：获取连续体素在图片上的投影
            ray_grid: (B, N, 2), Image coordinates in X,Y of image plane
            ray_depths: (B, N), Image depth
            voxel_grid：体素场上点的坐标
            lidar_grid：未进行数据增强的原始lidar坐标系下的坐标
            """

...
class VoxelFieldFusion(nn.Module):
    ...
    def fill(self, k, encoded_voxel_batch, render_indices_batch):
        # neighbor_idx = ops.knn(k, encoded_voxel_batch, render_indices_batch).transpose(-1,-2)
    ...
    def forward(self, batch_dict, encoded_voxel=None, encoded_feat2d=None, layer_name=None):
        ...
        for _idx in range(len(batch_dict['image_shape'])):  # _idx之的是batch的idx,对每个batch有
            ...
            if 'ray' in self.fuse_mode:
                ...
                np.savetxt('/home/zhanghaoming/visual/np_render_indices.txt',np_render_indices)
                np.savetxt('/home/zhanghaoming/visual/np_encoded_voxel.txt',np_encoded_voxel)
                
                if layer_name == 'layer1' or layer_name == 'layer2': # TODO
                    if render_indices.size(0) !=0 and render_feat.size(0) !=0:
    ##########################
                    # 这一batch中所有点的坐标(N,XYZ)
                        ...
                        for n in range(max_pos.size(0)):
                            ...
                            fill_voxel = fill_voxel.int().to(device=render_indices.device)
                            # 对扩展虚拟点的lidar空间位置信息进行编码
                            fill_voxel_feat = self.ray_blocks[layer_name](fill_voxel_feat.to(device=render_feat.device))

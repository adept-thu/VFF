...
##########################TODO
                ...
                if render_indices.size(0) !=0 and voxel_indices_select.size(0) !=0:
                    ...
                    render_indices  = self.index_points(render_indices.unsqueeze(0),ball_index).squeeze(0)
                    relative_dist = torch.sum((render_indices - voxel_indices_select.unsqueeze(1).repeat(1,k,1))**2,-1,keepdim=True).sqrt()
                    render_indices  = render_indices.reshape(-1,3)
                    ...
                    render_feat = (voxel_feat_select.unsqueeze(1).repeat(1,k,1) * relative_dist.repeat(1,1,render_feat.size(-1)) / r).reshape(-1,render_feat.size(-1))

...
class VoxelFieldFusion(nn.Module):
    def __init__(self, model_cfg, grid_size, pc_range, disc_cfg=None, device="cuda"):
        ...
        if 'ray' in self.fuse_mode:
            ...
            # self.fuse_true_blocks = nn.ModuleDict()
            ...
            for _layer in model_cfg.LAYER_CHANNEL.keys():
                ...
                self.fuse_blocks[_layer] = nn.Sequential(nn.Linear(in_features=out_channel*2,
                                                                   ...
                                                                   bias=True),
                                                         nn.ReLU())
                # self.fuse_true_blocks[_layer] = nn.Sequential(nn.Linear(in_features=out_channel*2,
                #                                                    out_features=out_channel,
                #                                                    bias=True),
                #                                          nn.ReLU())
                self.ic= out_channel
                rc = out_channel // 4
                self.conv1[_layer] = nn.Sequential(nn.Conv1d(self.ic, out_channel, 1),
                                    nn.BatchNorm1d(out_channel),
                                    nn.ReLU())
                self.fc1[_layer]  = nn.Linear(out_channel, rc)
                self.fc2[_layer]  = nn.Linear(out_channel, rc)
                self.fc3[_layer]  = nn.Linear(rc, 1)
    ...
    def fusion(self, image_feat, voxel_feat, image_grid,layer_name):
        """
        ...
        if 'sum' in self.fuse_mode:
            ...
            #TODO
            new_grid_feat=fuse_feat.unsqueeze(0)  #BNC
            batch = new_grid_feat.size(0)
            img_feas_f = new_grid_feat.contiguous().view(-1, self.ic) #BCN->BNC->(BN)C
            point_feas_f =voxel_feat.contiguous().view(-1, self.of) #BCN->BNC->(BN)C'
            # print(img_feas)
            ri = self.fc1[layer_name](img_feas_f)
            rp = self.fc2[layer_name](point_feas_f)
            att = F.sigmoid(self.fc3[layer_name](F.tanh(ri + rp))) #BNx1
            att = att.squeeze(1)
            att = att.view(batch, 1, -1) #B1N
            # print(img_feas.size(), att.size())
            img_feas_new = self.conv1[layer_name]( new_grid_feat)
            out = (img_feas_new * att)
            voxel_feat = voxel_feat + out.squeeze(0).transpose(0,1)
            # voxel_feat = torch.cat([voxel_feat.unsqueeze(0).transpose(1,2), out], dim=1)
            # voxel_feat = self.fuse_true_blocks[layer_name](voxel_feat).squeeze(0).transpose(0,1)
    ...
    def ray_render(self, ray_grid, ray_feat, image_grid, image_feat, shape, layer_name, topk_num, min_n=-1, max_n=1):   
        ...
        ##TODO
        new_grid_feat=grid_feat[ray_mask].unsqueeze(0)
        if(new_grid_feat.size(1)!=0):
            batch = new_grid_feat.size(0)
            img_feas_f = new_grid_feat.contiguous().view(-1, self.ic) #BCN->BNC->(BN)C
            point_feas_f = ray_feat[ray_mask].contiguous().view(-1, self.of) #BCN->BNC->(BN)C'
            # print(img_feas)
            ri = self.fc1[layer_name](img_feas_f)
            rp = self.fc2[layer_name](point_feas_f)
            att = F.sigmoid(self.fc3[layer_name](F.tanh(ri + rp))) #BNx1
            ...
            att = att.view(batch, 1, -1) #B1N
            # print(img_feas.size(), att.size())
            img_feas_new = self.conv1[layer_name]( new_grid_feat.transpose(1,2))
            out = (img_feas_new * att).squeeze(0).transpose(0,1)
        else:
            out= grid_feat[ray_mask]
    
        # render_feat = torch.cat([ray_feat[ray_mask], grid_feat[ray_mask]], dim=1)
        render_feat = torch.cat([ray_feat[ray_mask], out], dim=1)

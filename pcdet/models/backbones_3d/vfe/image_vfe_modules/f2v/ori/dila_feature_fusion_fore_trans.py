...
class VoxelFieldFusion(nn.Module):
    def __init__(self, model_cfg, grid_size, pc_range, disc_cfg=None, device="cuda"):
        ...
        if 'ray' in self.fuse_mode:
            ...
            self.pre_fuse_blocks = nn.ModuleDict()
            self.dila_blocks = nn.ModuleDict()
            ...
            for _layer in model_cfg.LAYER_CHANNEL.keys():  # 对每一层
                ...
                self.fuse_blocks[_layer] = nn.Sequential(nn.Linear(in_features=out_channel * 2,
                                                                   ...
                                                                   bias=True),
                                                         nn.ReLU())
                self.pre_fuse_blocks[_layer] = nn.Sequential(nn.Linear(in_features=out_channel * 2,
                                                                   out_features=out_channel,
                                                                   bias=True),
                                                         nn.ReLU())
                self.dila_blocks[_layer] = nn.Sequential(nn.Conv2d(in_channels=img_in_channel,
                                                                    out_channels=out_channel,
                                                                    kernel_size=5,
                                                                    dilation=2,
                                                                    stride=1,
                                                        ...
                                                        nn.BatchNorm2d(out_channel),
                                                         nn.ReLU())
                self.encoder_layer = nn.TransformerEncoderLayer(d_model=out_channel, nhead=4)
    ...
    def fusion(self, image_feat, voxel_feat, image_grid,layer_name):  # 对应公式(4)
        """
        ...
        if 'sum' in self.fuse_mode:
            fuse_feat = image_feat[:, image_grid[:, 0], image_grid[:, 1]] # (c,n)
            # voxel_feat += fuse_feat.permute(1, 0)
            voxel_feat = self.pre_fuse_blocks[layer_name](torch.cat([voxel_feat,fuse_feat.permute(1, 0)],dim=-1))
...
################
            image_feat = self.dila_blocks[layer_name](image_feat.unsqueeze(0)).squeeze(0)
            voxel_feat[voxel_mask] = self.fusion(image_feat, voxel_feat[voxel_mask],
                                                 image_grid[point_mask],layer_name)  # 图片信息与点的信息融合
...
##########################TODO
                ...
                if render_indices.size(0) !=0 and voxel_indices_select.size(0) !=0:
                    voxel_feat[voxel_mask][sample_encoded_mask] = self.transformer_encoder(voxel_feat_select.unsqueeze(0)).squeeze(0)
                    encoded_voxel.features[index_mask] = voxel_feat

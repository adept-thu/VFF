...
class VoxelFieldFusion(nn.Module):
    def __init__(self, model_cfg, grid_size, pc_range, disc_cfg=None, device="cuda"):
        ...
        if 'ray' in self.fuse_mode:
            ...
            self.sample_background_blocks = nn.ModuleDict() # 背景bocks
            ...
            for _layer in model_cfg.LAYER_CHANNEL.keys():  # 对每一层
                ...
                sample_background_block = OrderedDict() # 背景bocks
                ...
                for _block in range(self.block_num):  # 每一层的各个块（3块）
                    ...
                    if "learnable" in self.ray_sample.METHOD:  # importance采样法
                        sample_blocks['sample_{}_conv_{}'.format(_layer, _block)] = nn.Conv2d(
                            ...
                            padding=1,
                            bias=True)
                        sample_background_block['sample_background_{}_conv_{}'.format(_layer, _block)] = nn.Conv2d(
                            in_channels=img_in_channel,
                            out_channels=out_channel if _block < self.block_num - 1 else 1,
                            kernel_size=3,
                            stride=1,
                    ...
                    if _block < self.block_num - 1:
                        ...
                        if "learnable" in self.ray_sample.METHOD:
                            ...
                            sample_background_block['sample_background_{}_bn_{}'.format(_layer, _block)] = nn.BatchNorm2d(out_channel)
                            sample_background_block['sample_background_{}_relu_{}'.format(_layer, _block)] = nn.ReLU()
                ...
                if "learnable" in self.ray_sample.METHOD:
                    ...
                    for _sample_background in sample_background_block:
                        if 'relu' in _sample_background or 'bn' in _img: continue
                        nn.init.normal_(sample_background_block[_sample_background].weight, mean=0, std=0.01)
                        if sample_background_block[_sample_background].bias is not None:
                            nn.init.constant_(sample_background_block[_sample_background].bias, 0)
                    self.sample_background_blocks[_layer] = nn.Sequential(sample_background_block)
                #  三个embeding层，都是MLP
    ...
    def forward(self, batch_dict, encoded_voxel=None, encoded_feat2d=None, layer_name=None):
        ...
        for _idx in range(len(batch_dict['image_shape'])):  # _idx之的是batch的idx,对每个batch有
            ...
            if 'ray' in self.fuse_mode:
                ...
                # voxel_indices_remain= voxel_indices[voxel_mask][~sample_back_mask]
                # voxel_feat_remain = voxel_feat[voxel_mask][~sample_back_mask]

                voxel_feat[voxel_mask] = torch.cat([voxel_feat[voxel_mask][~sample_back_mask], voxel_feat[voxel_mask][sample_back_mask] * 0.1], dim = 0)  # 背景压制
                encoded_voxel.features[index_mask] = voxel_feat
...
##########################TODO
                ...
                if render_indices.size(0) !=0 and voxel_indices_select.size(0) !=0:
                    ...
                    render_feat = self.index_points(render_feat.unsqueeze(0),ball_index).squeeze(0).reshape(-1,render_feat.size(-1))
...
##########################
                ...
                if back_prob is not None:
                    ...
                    back_grid_gt = torch.zeros_like(back_prob)

                    for _box in box2d_gt:
                        back_grid_gt[:, _box[1]:_box[3], _box[0]:_box[2]] = 1
                    back_pred.append(back_prob)
                    back_gt.append(back_grid_gt)
    ...
    def ray_render(self, ray_grid, ray_feat, image_grid, image_feat, shape, layer_name, topk_num,voxel_indices =None, encoded_voxel_feat = None, min_n=-1, max_n=1):
        ...
        if "learnable" in self.ray_sample.METHOD:  # 可学习的采样法
            ...
            #TODO 背景mask
            back_prob = self.sample_background_blocks[layer_name](image_feat)     # TODO 加了背景判断
    ...
    def get_loss(self, ray_pred, ray_gt, ray_multi, sample_pred, sample_gt, back_pred, back_gt):
        ...
        if len(back_pred) > 0:
            loss_back = self.sample_loss(back_pred, back_gt)

...
class VoxelFieldFusion(nn.Module):
    def __init__(self, model_cfg, grid_size, pc_range, disc_cfg=None, device="cuda"):
        ...
        self.k = model_cfg.get('K', None)  # ray_sum
        self.r = model_cfg.get('R', None)  # ray_sum
    ...
    def forward(self, batch_dict, encoded_voxel=None, encoded_feat2d=None, layer_name=None):
        ...
        for _idx in range(len(batch_dict['image_shape'])):  # _idx之的是batch的idx,对每个batch有
            ...
            if not self.image_interp: # 如果没有插值，就将image扩大成feature map的大小
            ...
            if 'ray' in self.fuse_mode and layer_name=="layer1":
...
##########################TODO
                ...
                if render_indices.size(0) !=0 and voxel_indices_select.size(0) !=0:
                    ...
                    encoded_voxel.features[index_mask] = voxel_feat
                    ball_index = ball_query(0,layer_spacific_r,layer_spacific_k,render_indices.unsqueeze(0).float(),voxel_indices_select.unsqueeze(0).float())
...
##########################
        ...
        if self.training and layer_name=="layer1" and self.loss_cfg is not None:
